...
# Purpose: Reset local development foundation stack including volumes.
# Examples:
#   bash infra/environments/dev/scripts/run/reset.sh
#   bash infra/environments/dev/scripts/run/reset.sh
set -euo pipefail
ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../../.." && pwd)"
docker compose -f "$ROOT_DIR/infra/environments/dev/docker/compose/stack.foundation.yml" down -v --remove-orphans
