...
# Purpose: Validate all GCP overlays can render with kustomize.
# Examples:
#   bash infra/lifecycle/verify/verify-manifests-gcp.sh
#   KUBECONFIG=~/.kube/config bash infra/lifecycle/verify/verify-manifests-gcp.sh
set -euo pipefail
...
for env in dev test prod; do
  kustomize_render "$ROOT_DIR/infra/environments/gcp/k8s/overlays/$env" >/dev/null
done

echo "GCP overlays validated"
